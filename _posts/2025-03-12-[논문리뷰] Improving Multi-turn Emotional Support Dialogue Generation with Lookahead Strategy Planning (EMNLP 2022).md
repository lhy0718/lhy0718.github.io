...
- 또 다른 도전은 대화 중 사용자의 상태를 동적으로 모델링하는 것임.
  - 기존 감정 관련 대화 작업은 사용자의粗粒도 감정 유형을 감지하여 대화 생성을 향상시키려 했음 (Lin et al., 2019; Majumder et al., 2020; Li et al., 2020a).
...
- 본 논문에서는 다중 턴 ESC 시스템인 MultiESC를 제안함. 
  ...
  - 사용자 상태 모델링에서 NRC V AD 어휘를 이용해 사용자의 미세한 감정 표현과 원인을 포착함.
...
## 한계
- 제안된 방법이 기존 기준선에 비해 큰 개선을 보이기는 했지만, 감정 지원 챗봇에 대한 연구는 아직 갈 길이 멀다고 생각함.
- 챗봇의 발화는 일반적이고 반복적인 경우가 많아, 사용자 경험에 대한 개인화된 깊은 이해나 구체적이고 건설적인 제안을 제공하지 못함.
- 상식 지식의 통합을 통해 이 문제를 완화할 수 있을 것으로 생각하며, 이는 향후 연구 방향이 될 것임.
  
## 윤리적 고려사항
- 본 논문에서의 "감정 지원"은 주로 친구나 가족과의 대화와 같은 동료 지원을 의미하며, 전문적인 심리 상담이나 치료를 제공하는 챗봇을构造할 수 있다고 주장하지 않음.
- 이러한 시스템을 사용할 때에는 주의가 필요하며, 자해나 자살 경향이 있는 사용자 탐지를 위한 위기 경고 메커니즘이 바람직함. 
- 실험 데이터셋인 ESConv는 민감한 정보를 필터링하여 적절하게 구성됨.
