...
- 자원 제약 때문에 최적화 접근법(예: 정량화)이 필요함.
- 여러 연구가 LLM의 성격을 다루었지만, 로컬에서 배포되는 RPDA의 정량화가 행동에 미치는 영향을 조사한 연구는 없음.
- 본 연구는 정량화된 버전의 LLM에서 구축된 RPDA 성격(QRPDA)의 일관성을 조사함.
- 다음과 같은 연구 질문(RQ)을 설정함:
  - RQ1: LLM의 정량화가 QRPDAs의 성격 일관성에 어떤 영향을 미치는가?
  - RQ2: QRPDAs의 성격 일관성을 개선할 수 있는 전략은 무엇인가?
  - RQ3: 로컬 배포된 QRPDA를 위한 최적의 모델 크기, 유형, 정량화 조합은 무엇인가?
- 다양한 LLM과 정량화 수준을 사용한 실험을 설계하여 RQ를 해결함.
- 연구 결과, 정량화가 성격 일관성을 감소시키며, 대화 중에 할당된 특성을 유지하는 데 도전 과제가 있다는 것을 나타냄.
- 성격 변화를 해결하기 위해 "Think2"라는 비모수적 접근법을 제안하며, 이는 정량화된 대화 에이전트의 일관성을 유지하는 데 좋은 결과를 보여줌.
...
- **정량화 방법**: 
  - GGUF 정량화 방법에만 국한; 다른 정량화 기법 (AWQ, GPTQ 등) 탐색 필요
