...
### 제한 사항
- 잘못된 라벨링을 가진 질문을 완전히 제거할 수 없음.
- 단일 의도 설명을 선택해야 하므로, 여러 올바른 답이 존재하는 경우 문제 발생.
- 데이터셋의 면적 행동 분포가 상대적으로 희소하여 LLM의 의도 감지 능력을 전방위적으로 측정하기 어려움.

### 윤리적 고려 사항
- LLM의 의도 감지 능력을 평가하고자 하며, 심각한 윤리적 영향이 있을 것으로 기대하지 않음.
- 정확한 의도 감지가 가능한 LLM이 드러난다면, 더 광범위한 분야에 활용될 가능성 존재.
- LLM을 통해 사람들의 의도를 교란시킬 위험이 있으며, 악의적인 행동으로 사용될 가능성에 대해 경계해야 함.
