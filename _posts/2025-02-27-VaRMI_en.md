...
- **Training Approach**:
  ...
  - The objective is defined as $$ J(\theta) = E*{\tau \sim p(\pi*{\theta}(\tau))} \left[ \sum_{t=0}^{T} \gamma^{t} r(s_{t}, a_{t}) \right] $$.
...
- **Sample Collection**:
  ...
  - Importance weights $$ w*{t} = \prod*{t'}=0^{t} \frac{\pi*{\theta}(a*{t'}|s*{t'})}{\pi*{b}(a*{t'}|s*{t'})} $$ are approximated for variance reduction.

- **Importance Sampling Methods**:
  1. **GOLD Method**:
     - Assumes constant likelihood for all training samples under $$ \pi*b $$ allowing simplification to $$ w*{t} = \pi*{\theta}(a*{t}|s\_{t}) $$.
