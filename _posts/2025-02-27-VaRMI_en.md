...
- **Sample Collection**:
  ...
  - Importance weights $$ w_{t} = \prod_{t}^{t'=0} \frac{\pi_{\theta}(a_{t'}|s_{t'})}{\pi_{b}(a_{t'}|s_{t'})} $$ are approximated for variance reduction.

- **Importance Sampling Methods**:
  1. **GOLD Method**:
     - Assumes constant likelihood for all training samples under $$ \pi*b $$ allowing simplification to $$ w_{t} = \pi_{\theta}(a_{t}|s_{t}) $$.
