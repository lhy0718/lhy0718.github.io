...
- **정의**: 
  ...
  - 이때 S는 m개의 단어로 구성된 문장이며, T는 S의 부분 последователь는 n개의 단어로 이루어져 있다.
...
- 수식 예시:
  ...
  - 지식 확대를 위한 목표는 $$ G(\mathcal{K}) = \sum_{i=1}^{n} P(y_i|K) $$으로 나타낼 수 있음, 여기서 $K$는 지식 집합을 의미.
...
- 주어진 문장-측면 쌍 {S, T}에 대해:
  ...
  - 각 단어 $w_i$를 임베딩 행렬 $E \in \mathbb{R}^{|V| \times d_w}$로 저차원 벡터 공간에 임베드.
    - 여기서 $|V|$는 어휘의 크기, $d_w$는 단어 임베딩의 차원.
