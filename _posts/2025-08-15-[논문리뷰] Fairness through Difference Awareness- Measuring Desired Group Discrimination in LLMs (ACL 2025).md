...
본 논문은 알고리즘 공정성을 색맹적 차별 회피에 의존하기보다 맥락에 따라 그룹 간 차이를 인지하는 차이 인식(diff awareness)을 제안한다. 설명적/규범적/상관적 벤치마크를 구분하고 여덟 가지 시나리오로 구성된 16k개 질문의 벤치마드를 통해 차이 인식의 공정성 차원을 평가하며, 열 가지 모델 실험에서 차이 인식이 기존 편향 완화 전략에 역효과를 낼 수 있음을 보인다.
